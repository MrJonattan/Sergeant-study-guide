...
// ─── Sergeant Focus Callouts Tests ───

test('sergeantFocus callouts exist and have content', () => {
  let totalCallouts = 0;
  let errors = [];
  D.chapters.forEach(ch => {
    if (ch.sergeantFocus && Array.isArray(ch.sergeantFocus)) {
      totalCallouts += ch.sergeantFocus.length;
      ch.sergeantFocus.forEach(callout => {
        if (!callout.text || callout.text.length < 20) {
          errors.push(`${ch.id}: callout text too short`);
        }
        if (!callout.filename) {
          errors.push(`${ch.id}: callout missing filename`);
        }
      });
    }
  });
  // Expect at least 200 callouts (allows for small drift from documented 202)
  assert(totalCallouts >= 200, `Expected >= 200 Sergeant Focus callouts, got ${totalCallouts}`);
  assert(errors.length === 0, errors.join('\n'));
});

test('sergeantFocus chapters have questions for quiz', () => {
  const chaptersWithCallouts = D.chapters.filter(ch => ch.sergeantFocus && ch.sergeantFocus.length > 0);
  let chaptersWithQuestions = 0;
  let totalQuestions = 0;
  chaptersWithCallouts.forEach(ch => {
    if (ch.questions && ch.questions.length > 0) {
      chaptersWithQuestions++;
      totalQuestions += ch.questions.length;
    }
  });
  // At least some chapters with callouts should have questions
  assert(chaptersWithQuestions > 0, 'No chapters with Sergeant Focus callouts have questions');
